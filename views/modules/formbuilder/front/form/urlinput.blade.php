...
<div class="form-group<?php if ($required) {
  ...
  <div class="<?php echo $inputsize ?>">
    <input id="<?php echo $name ?>" name="<?php echo $name ?>" value="{!! old($name) !!}" type="url" placeholder="<?php echo $placeholder ?>" class="browser-default form-control <?php echo $inputheight ?>" <?php if ($required) {
    ?> required <?php 
} ?> />
    <?php if ($helptext) {
