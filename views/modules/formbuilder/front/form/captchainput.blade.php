...
<div class="form-group required-control {{ $errors->has($name) ? ' has-error' : '' }}">
  ...
  <div class="<?php echo $inputsize ?>">
    ...
    <input id="<?php echo $name ?>" name="<?php echo $name ?>" value="{!! old($name) !!}" type="text" placeholder="<?php echo $placeholder ?>" class="browser-default form-control <?php echo $inputheight ?>" required/>
    <?php if ($helptext) {
