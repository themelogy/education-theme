...
<div class="form-group<?php if ($required) {
  ...
  <div class="<?php echo $inputsize ?>">
    <input id="<?php echo $name ?>" name="<?php echo $name ?>" value="{!! old($name) !!}" type="date" placeholder="<?php echo $placeholder ?>" class="browser-default datepicker form-control <?php echo $inputheight ?>" <?php if ($required) {
    ?> required <?php 
} ?> />
    <?php if ($helptext) {
