...
@push('js_inline')
    <script>
        var hrConfig = {
            ...
            hasCaptcha: {{ setting('hr::user-login') ? 'true' : 'false' }},
            bearerToken: 'Bearer {{ $currentUser ? $currentUser->getFirstApiKey() : '' }}',
            locales: {
                buttons: {
                    action: '{{ trans('hr::applications.buttons.create') }}'
                }
            }
        };
        Vue.config.devtools = {{ App::environment()=='local' ? 'true' : 'false' }};
        Vue.prototype.$http = axios;
        window.axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
        window.axios.defaults.headers.common['X-CSRF-TOKEN']     = hrConfig.csrf_token;
        window.axios.defaults.headers.common['Authorization']    = hrConfig.bearerToken;
        ...
        new Vue({
            ...
            created: function() {
                this.authorization_key = hrConfig.csrf_token;
                this.hasCaptcha        = hrConfig.hasCaptcha;
                this.button            = hrConfig.locales.buttons.action;
                this.newApplication    = JSON.parse(JSON.stringify(this.application));
            },
            methods: {
