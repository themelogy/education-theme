...
<div class="row">
    <fieldset>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group{{ $errors->has("settings.tracking_code") ? ' has-error' : '' }}">
                    {!! Form::label("settings.tracking_code", "Google Dönüşüm Kodu".':') !!}
                    {!! Form::textarea('settings[tracking_code]', !isset($page->settings->tracking_code) ? '' : $page->settings->tracking_code, ['class'=>'form-control']) !!}
                    {!! $errors->first("settings.tracking_code", '<span class="help-block">:message</span>') !!}
                </div>
            </div>
        </div>
    </fieldset>
